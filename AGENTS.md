...
- **API Provider**: `personal-vision-services` è il progetto GCP dedicato a **tutti i servizi API esterni** (Gemini, Vision). Possiede API Key, billing e quote. Il Hub usa le chiavi di PVS come proxy, NON possiede servizi AI.
- **Region**: ALWAYS use **`europe-west8` (Milan)** for everything.
- **Credentials**: Bot secrets (`TELEGRAM_TOKEN`, `ORCHESTRATOR_SECRET`) in GCP Secret Manager di `bot-orchestrator-hub`. API keys AI (`GEMINI_API_KEY`) create in `personal-vision-services` e montate come env var sul Cloud Run del Hub.
