...
3. **Il Progetto API (Servizi Centralizzati)**: `personal-vision-services`
    ...
    - **Separazione**: Il Hub non "possiede" servizi AI — li **proxy** utilizzando le chiavi create in `personal-vision-services`. Questo garantisce un unico punto di fatturazione, gestione quote e configurazione per ogni servizio esterno, indipendentemente da quanti progetti li consumano.
...
| **Personal Vision Services** | `personal-vision-services` | API Key (Gemini, Vision), billing servizi AI, budget alerts, quota management | Container, servizi compute, bot logic |
| **Script Spese** | Google Workspace | Apps Script, Sheets, Drive, Gmail (runtime satellite) | Nessuna risorsa GCP diretta |

> **🚫 ERRORE DA EVITARE**: Non creare API Key per servizi AI (Gemini, Vision) dentro `bot-orchestrator-hub`. Tutte le chiavi API devono essere create in `personal-vision-services` e poi montate come env var sul Cloud Run del Hub. Questo mantiene la separazione dei costi: se un domani si aggiunge un nuovo progetto che usa AI, le quote e i costi rimangono centralizzati in un unico posto.

**Come funziona concretamente:**

```text
┌─────────────────────────────────────┐
│    personal-vision-services         │
...
| `GEMINI_API_KEY` | API Key: `GeminiAIStudioKey` | `personal-vision-services` | Env Var |

> **Nota**: La `GEMINI_API_KEY` è creata nel progetto `personal-vision-services` (owner del billing AI) ma montata come variabile d'ambiente nel Cloud Run di `bot-orchestrator-hub`. Questo mantiene la separazione tra bot (hub) e servizi AI (PVS).
